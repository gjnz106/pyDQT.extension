...
Supports: Walls, Floors, Ceilings, Roofs, Columns, Structural Columns, Beams, Structural Framing, Structural Foundations, Doors, Windows
...
__author__ = "DQT"
__doc__ = """Automatically adjusts Base Constraint when changing Base Constraint
...
def is_foundation(elem):
    """Check if element is a structural foundation"""
    if not isinstance(elem, FamilyInstance):
        return False
    ...
    try:
        ...
        cat_id = _eid_int(category.Id)
        # Check if it's Structural Foundations category
        return cat_id == BuiltInCategory.OST_StructuralFoundation.value__
    except:
        return False

def is_door(elem):
    """Check if element is a door"""
    if not isinstance(elem, FamilyInstance):
        return False
    ...
    try:
        ...
        cat_id = _eid_int(category.Id)
        # Check if it's Doors category
        return cat_id == BuiltInCategory.OST_Doors.value__
    except:
        return False
...
def is_window(elem):
    ...
    try:
        ...
        cat_id = _eid_int(category.Id)
        # Check if it's Windows category
        return cat_id == BuiltInCategory.OST_Windows.value__
    except:
        return False
...
def get_element_base_info(elem):
    ...
    Works for Wall, Floor, Ceiling, Roof, Column, Beam, Foundation, Door, Window"""
    try:
        ...
        # Ceiling
            ...
            base_constraint_param = elem.get_Parameter(BuiltInParameter.LEVEL_PARAM)
            base_offset_param = elem.get_Parameter(BuiltInParameter.CEILING_HEIGHTABOVELEVEL_PARAM)
        
        # Roof
        elif isinstance(elem, (RoofBase, FootPrintRoof, ExtrusionRoof)):
            base_constraint_param = elem.get_Parameter(BuiltInParameter.ROOF_CONSTRAINT_LEVEL_PARAM)
            base_offset_param = elem.get_Parameter(BuiltInParameter.ROOF_LEVEL_OFFSET_PARAM)
        ...
        elif is_foundation(elem):
            base_constraint_param = elem.get_Parameter(BuiltInParameter.FAMILY_LEVEL_PARAM)
            base_offset_param = elem.get_Parameter(BuiltInParameter.INSTANCE_ELEVATION_PARAM)
    ...
    try:
        ...
        if selection.Count > 0:
            ...
            for elem_id in selection:
                ...
                if (isinstance(elem, (Wall, Floor, Ceiling, RoofBase, FootPrintRoof, ExtrusionRoof)) or 
                    is_column(elem) or is_beam(elem) or is_foundation(elem) or 
                    is_door(elem) or is_window(elem)):
            ...
            try:
                ...
                class ElementSelectionFilter(ISelectionFilter):
                    def AllowElement(self, elem):
                        return (isinstance(elem, (Wall, Floor, Ceiling, RoofBase, FootPrintRoof, ExtrusionRoof)) or 
                                is_column(elem) or is_beam(elem) or is_foundation(elem) or 
                                is_door(elem) or is_window(elem))
                ...
                refs = uidoc.Selection.PickObjects(
                    ...
                    "Select elements to adjust Base Constraint"
                )
