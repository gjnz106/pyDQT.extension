...
class LevelSelectionForm(Form):
    ...
    def InitializeComponent(self):
        ...
        # --- Footer ---
        footer = Label()
        footer.Text = "Copyright (c) 2025 by Dang Quoc Truong (DQT)"
    ...
    for dep_id in dep_ids:
        ...
        for param in fi.Parameters:
            ...
            if storage == StorageType.Double:
                ...
                val = ('id', param.AsElementId().IntegerValue)
...
def recreate_hosted_element(hosted_info, host_wall):
    ...
    symbol = doc.GetElement(ElementId(hosted_info['symbol_id'].IntegerValue))
    if symbol is None:
        return None
...
def main():
    ...
    # Execute
    t = Transaction(doc, "Split Walls at Levels")
    t.Start()
    ...
    try:
        ...
        for wi in wall_info_list:
            wall = doc.GetElement(ElementId(wi['element_id']))
            if wall is None:
