...
# CRITICAL: Import WPF Grid BEFORE Revit wildcard import (which overwrites Grid)
...
def _hex_to_brush(hex_color):
    ...
    Uses BrushConverter which is reliable across all IronPython versions.
    Avoids Color.FromArgb/Color.FromRgb which are unreliable in IronPython."""
...
class BatchRenameDialog(Window):
    ...
    def _create_template_info_tab(self):
        ...
        self.remove_current_name_check.Foreground = _hex_to_brush(Config.ERROR_COLOR)
    ...
    def _create_preview_section(self):
        border = Border()
        border.BorderBrush = _hex_to_brush(Config.BORDER_COLOR)
        ...
        label.Background = _hex_to_brush(Config.PRIMARY_COLOR)
        WPFGrid.SetRow(label, 0)
        grid.Children.Add(label)
        
        scroll = ScrollViewer()
        scroll.VerticalScrollBarVisibility = System.Windows.Controls.ScrollBarVisibility.Auto
        scroll.Background = _hex_to_brush(Config.WHITE)
    ...
    def _create_buttons(self):
        ...
        apply_btn.Background = _hex_to_brush(Config.SUCCESS_COLOR)
        apply_btn.Foreground = _hex_to_brush(Config.WHITE)
    ...
    def _update_preview(self, sender=None, args=None):
        ...
        if changed_count > 0:
            ...
            self.preview_text.Foreground = _hex_to_brush("#333333")
        else:
...
class ViewTemplateManagerWindow(Window):
    ...
    def _create_header(self):
        ...
        subtitle = TextBlock()
        subtitle.Text = "by Dang Quoc Truong (DQT) | Revit 2024-2026"
    ...
    def _create_card(self, label, value, margin_index, value_color=None):
        border = Border()
        border.Background = _hex_to_brush(Config.WHITE)
        border.BorderBrush = _hex_to_brush(Config.BORDER_COLOR)
        border.BorderThickness = Thickness(1)
        border.CornerRadius = System.Windows.CornerRadius(4)
        ...
        label_text.Foreground = _hex_to_brush(Config.TEXT_LIGHT)
        ...
        if value_color:
            value_text.Foreground = _hex_to_brush(value_color)
    ...
    def _create_left_panel(self):
        ...
        btn_all.Background = _hex_to_brush(Config.WHITE)
        ...
        btn_clear.Background = _hex_to_brush(Config.WHITE)
        ...
        btn_unused.Background = _hex_to_brush(Config.WHITE)
    ...
    def _create_datagrid(self):
        ...
        # NOTE: DataGridCheckBoxColumn in IronPython edit mode is unreliable.
        # Using IsReadOnly=True with row-level SelectionMode=Extended instead.
        # Selection is handled via DataGrid.SelectedItems (Ctrl+Click / Shift+Click).
        
        col_name = DataGridTextColumn()
        col_name.Header = "Template Name"
        col_name.Binding = Binding("name")
        col_name.Width = DataGridLength(350)
    ...
    def _create_action_buttons(self):
        ...
        btn_batch = self._create_button("Batch Rename", Config.PRIMARY_COLOR)
        btn_batch.Click += self._on_batch_rename
        center_panel.Children.Add(btn_batch)
        
        btn_duplicate = self._create_button("Duplicate", Config.WARNING_COLOR, white_fg=True)
        btn_duplicate.Click += self._on_duplicate
        center_panel.Children.Add(btn_duplicate)
        
        btn_delete = self._create_button("Delete", Config.ERROR_COLOR, white_fg=True)
        ...
        btn_refresh.Background = _hex_to_brush(Config.WHITE)
    ...
    def _create_button(self, text, bg_color, white_fg=False):
        btn = Button()
        btn.Content = text
        btn.Padding = Thickness(10, 5, 10, 5)
        btn.Margin = Thickness(2)
        btn.Background = _hex_to_brush(bg_color)
        
        if white_fg:
            btn.Foreground = _hex_to_brush(Config.WHITE)
    ...
    def _create_footer(self):
        ...
        text.Foreground = _hex_to_brush(Config.TEXT_DARK)
    ...
    def _get_selected_items(self):
        """Get selected items from DataGrid.SelectedItems (row-level selection)."""
        try:
