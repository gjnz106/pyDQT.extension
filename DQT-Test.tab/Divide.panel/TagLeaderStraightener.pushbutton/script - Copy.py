...
def safe_set_elbow(tag, elbow_point):
    """Set the elbow point on a tag leader - returns True if success"""
    
    # === SPATIAL ELEMENT TAGS (Room/Area/Space) ===
    # These use SpatialElementTag API: LeaderElbow property directly
    if is_spatial_tag(tag):
        try:
            # For SpatialElementTag: set LeaderElbow directly
            # Some versions need HasElbow, some don't
            tag.LeaderElbow = elbow_point
            return True
        except:
            pass
        try:
            # Try with HasLeader first
    ...
    # === INDEPENDENT TAGS ===
    # Method 1: Revit 2022+ SetLeaderElbow with TaggedReference
    try:
        if hasattr(tag, 'GetTaggedReferences'):
            refs = tag.GetTaggedReferences()
            if refs.Count > 0:
                ...
                if hasattr(tag, 'HasLeader') and not tag.HasLeader:
                    tag.HasLeader = True
                # For IndependentTag: need LeaderEndCondition = Free for elbow to work
    ...
    # Method 2: Direct LeaderElbow property (older API)
    try:
        if hasattr(tag, 'LeaderEndCondition'):
            tag.LeaderEndCondition = LeaderEndCondition.Free
        tag.LeaderElbow = elbow_point
...
def analyze_tag(tag):
    ...
    if elbow:
        # Has elbow - check each segment
        seg1_diag = needs_straightening(head, elbow)
        seg2_diag = needs_straightening(elbow, end)
        is_diag = seg1_diag or seg2_diag
        dev1 = calc_deviation_angle(head, elbow)
        dev2 = calc_deviation_angle(elbow, end)
        max_dev = max(dev1, dev2)
    else:
        # No elbow - single straight leader
        is_diag = needs_straightening(head, end)
        max_dev = calc_deviation_angle(head, end)
    
    return is_diag, max_dev
...
def compute_elbow(head, end, mode="auto"):
    ...
    if mode == "horizontal_first":
        ...
        dx = abs(head.X - end.X)
        dy = abs(head.Y - end.Y)
        return elbow_hf if dx >= dy else elbow_vf
...
def straighten_one_tag(tag, mode="auto"):
    ...
    # Check if already has proper orthogonal elbow
    existing_elbow = safe_get_elbow(tag)
    if existing_elbow:
        # Has elbow - check if both segments are axis-aligned
        if not needs_straightening(head, existing_elbow) and not needs_straightening(existing_elbow, end):
            return True, "Already orthogonal"
    else:
        # No elbow - check if single leader is axis-aligned
    ...
    try:
        ok = safe_set_elbow(tag, elbow_point)
        if ok:
            return True, "OK"
        else:
            spatial = is_spatial_tag(tag)
            tag_type = "SpatialElementTag" if spatial else "IndependentTag"
            return False, "Cannot set elbow on {} (ID:{})".format(tag_type, tag.Id.IntegerValue)
...
class TagLeaderWindow:
    ...
    def on_apply(self, sender, args):
        ...
        msg = "Straightened: {} / {} tags".format(ok_count, len(selected))
        if fail_count > 0:
            msg += "\nFailed: {}".format(fail_count)
            if fail_msgs:
                msg += "\n\nReasons:\n- " + "\n- ".join(fail_msgs[:5])
        
        self.txtStatus.Text = "Done: {} OK, {} failed".format(ok_count, fail_count)
