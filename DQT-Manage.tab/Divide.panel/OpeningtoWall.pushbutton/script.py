...
class IntersectionResult:
    ...
    def build_tracking_key(self):
        mep_int = self.mep_id.IntegerValue if self.mep_id else 0
        wall_int = self.wall_id.IntegerValue if self.wall_id else 0
    ...
    for li_idx, link_inst in enumerate(link_instances):
        try:
            ...
            try:
                ...
                link_name = str(link_inst.Id.IntegerValue)
                    ...
                    try:
                        ...
                        for wall in walls:
                            try:
                                ...
                                # STEP 3: Fallback for fittings ONLY (no location curve)
                                if int_pt is None and not mep_line_start:
                                    mep_center = XYZ(
                                        (actual_min.X + actual_max.X) / 2.0,
                                        (actual_min.Y + actual_max.Y) / 2.0,
                                        (actual_min.Z + actual_max.Z) / 2.0
                                    )
                                    dist_to_wall = abs((mep_center - ws).DotProduct(wn))

                                    if dist_to_wall <= wall_half_t:
                                        corners = [
                                            XYZ(actual_min.X, actual_min.Y, mep_center.Z),
                                            XYZ(actual_max.X, actual_min.Y, mep_center.Z),
                                            XYZ(actual_min.X, actual_max.Y, mep_center.Z),
                                            XYZ(actual_max.X, actual_max.Y, mep_center.Z),
                                        ]
                                        dists = [(c - ws).DotProduct(wn) for c in corners]
                                        has_pos = any(d > wall_half_t for d in dists)
                                        has_neg = any(d < -wall_half_t for d in dists)

                                        if has_pos and has_neg:
                                            int_pt = mep_center - wn.Multiply((mep_center - ws).DotProduct(wn))
                                            param_on = (int_pt - ws).DotProduct(wd)
                                            if param_on < -0.05 or param_on > wc.Length + 0.05:
                                                int_pt = None
                                            else:
                                                passes_through = True
                                ...
                                try:
                                    ...
                                    r.wall_name = "Wall #" + str(wall.Id.IntegerValue)
                                try:
        ...
        try:
            ...
            try:
                ...
                if opening:
                    ...
                    r.status = "Updated (ID:" + str(opening.Id.IntegerValue) + ")"
    ...
    for r in selected_new:
        ...
        try:
            ...
            try:
                ...
                if opening:
                    write_opening_tag(opening, r.build_tag_string())
                    r.status = "Created (ID:" + str(opening.Id.IntegerValue) + ")"
                    r.existing_opening_id = opening.Id
                    created += 1
                    t.Commit()
                else:
...
class AutoOpeningWindow(Window):
        ...
        for lnk in links:
            try:
                ...
                try:
                    ...
                    name = "Link #" + str(lnk.Id.IntegerValue)
    ...
    def _build_row(self, idx, r, is_orphan):
        s = r.status
        if "Created" in s or "Updated" in s:
            bg = HIGHLIGHT
        elif "Deleted" in s:
            bg = LIGHT_GRAY
        elif "Error" in s:
        ...
        for i, t in enumerate(texts):
            ...
            tb.IsHitTestVisible = False  # pass clicks through to parent border
    ...
    def _on_cb_changed(self, sender, args):
        """Normal checkbox toggle (no shift)"""
        tag = sender.Tag
        if tag is None:
            return
        idx, is_orphan = tag
    ...
    def _on_row_click(self, sender, args):
        """Click on row border - handles Shift+Click range select"""
        tag = sender.Tag
        if tag is None:
            return
        idx, is_orphan = tag
        ...
        if shift and self._last_clicked_idx >= 0 and self._last_clicked_idx != idx:
            # Range select: toggle all between last and current to match current state
            all_items = self.results + self.orphan_results
            # Determine target state: opposite of current item
            target_item = all_items[idx] if 0 <= idx < len(all_items) else None
            if target_item is None:
                return
            new_state = not target_item.is_selected

            start = min(self._last_clicked_idx, idx)
            end = max(self._last_clicked_idx, idx)
            for i in range(start, end + 1):
                if 0 <= i < len(all_items):
                    all_items[i].is_selected = new_state

            self._last_clicked_idx = idx
            self._refresh_table()
            return

        # No shift: just toggle the single item
        all_items = self.results + self.orphan_results
        if 0 <= idx < len(all_items):
            all_items[idx].is_selected = not all_items[idx].is_selected
        self._last_clicked_idx = idx
        self._refresh_table()
    ...
    def _on_highlight(self, s, e):
        sel = [r for r in self.results if r.is_selected and r.wall_id]
        if not sel:
            TaskDialog.Show("Auto Opening", "No items selected.")
            return
        ids = list(set(r.wall_id for r in sel))
    ...
    def _on_zoom(self, s, e):
        ...
        for wid in ids:
            ...
            if not w:
                continue
            bb = w.get_BoundingBox(doc.ActiveView)
            if not bb:
        ...
        if not found:
            self.status_text.Text = "No bounding box found"
            return
        pad = 2.0
        try:
            uiview = None
            ...
            if uiview:
                ...
                self.status_text.Text = "Zoomed to " + str(len(ids)) + " walls"
            else:
                uidoc.ShowElements(id_list)
                self.status_text.Text = "Focused on " + str(len(ids)) + " walls"
        except:
            try:
                uidoc.ShowElements(id_list)
                self.status_text.Text = "Focused on " + str(len(ids)) + " walls"
