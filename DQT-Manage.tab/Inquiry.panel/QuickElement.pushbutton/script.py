...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="Quick Select Manager v2.0 - DQT"
...
class ElementItem(System.Object):
    ...
    def __init__(self, element, doc):
        ...
        self.element_id = str(element.Id.IntegerValue)
...
class ElementCollector(object):
    ...
    def collect(self, display_mode="Active View", filter_type="All Elements"):
        ...
        print("DEBUG: Display mode = {}".format(display_mode))
        print("DEBUG: Filter type = {}".format(filter_type))
        print("DEBUG: Categories count = {}".format(len(categories)))
                ...
                for elem in collector:
                    if elem and elem.Id:
                        try:
                            ...
                            print("DEBUG: Error creating ElementItem: {}".format(str(ex)))
                
                if count > 0:
                    print("DEBUG: Category {} = {} elements".format(bic, count))
                    
            except Exception as ex:
                print("DEBUG: Error with category {}: {}".format(bic, str(ex)))
        
        # Handle current selection
        if display_mode == "Current Selection":
            selection = self.uidoc.Selection.GetElementIds()
            print("DEBUG: Current selection count = {}".format(selection.Count))
            for elem_id in selection:
                try:
                    ...
                    print("DEBUG: Error with selection: {}".format(str(ex)))
        
        print("DEBUG: Total elements collected = {}".format(len(elements)))
        return elements
...
class QuickSelectWindow(Window):
    ...
    def _find(self, name):
        ...
        print("DEBUG: Could not find control: {}".format(name))
        return None
    
    def _get_controls(self):
        """Get references to UI controls"""
        print("DEBUG: Getting controls...")
        ...
        # Debug output
        print("DEBUG: dataGrid = {}".format(self.dataGrid))
        print("DEBUG: lstCategories = {}".format(self.lstCategories))
        print("DEBUG: cmbDisplay = {}".format(self.cmbDisplay))
    ...
    def _setup_events(self):
        ...
        if self.dataGrid:
            ...
            self.dataGrid.CellEditEnding += self._on_cell_edit_ending
    ...
    def _apply_filters(self):
        ...
        print("DEBUG: all_items count = {}".format(len(self.all_items)))
        ...
        if search_text:
            ...
            print("DEBUG: After search filter = {}".format(len(self.filtered_items)))
        
        # Category filter
        if self.lstCategories and self.lstCategories.SelectedItem:
            try:
                selected_tag = self.lstCategories.SelectedItem.Tag
                print("DEBUG: Selected category tag = {}".format(selected_tag))
                if selected_tag and selected_tag != "ALL":
                    self.filtered_items = [
                        item for item in self.filtered_items
                        if item.category == selected_tag
                    ]
                    print("DEBUG: After category filter = {}".format(len(self.filtered_items)))
            except Exception as ex:
                print("DEBUG: Category filter error: {}".format(str(ex)))
        
        print("DEBUG: Final filtered_items = {}".format(len(self.filtered_items)))
    ...
    def _update_datagrid(self):
        ...
        if not self.dataGrid:
            print("DEBUG: dataGrid is None!")
            return
        
        print("DEBUG: _update_datagrid called with {} items".format(len(self.filtered_items)))
        ...
        try:
            ...
            print("DEBUG: DataGrid ItemsSource set with {} items".format(observable.Count))
            
        except Exception as ex:
            print("DEBUG: _update_datagrid error: {}".format(str(ex)))
            traceback.print_exc()
    ...
    def _on_cell_edit_ending(self, sender, args):
        """Handle cell edit ending - update stats after checkbox change"""
        # Deferred update to allow binding to complete
        def update():
            try:
                self._update_stats()
            except:
