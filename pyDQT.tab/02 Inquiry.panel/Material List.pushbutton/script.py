# -*- coding: utf-8 -*-
"""
Material Manager (Standalone)
Manage Materials with beautiful UI - Sheet Manager Style

Compatible: Revit 2024 / 2025 / 2026 / 2027  (pyRevit 6.1, IronPython)
...
__doc__ = "Manage and analyze Materials - By DQT"
...
from System.Windows import (Window, Thickness, GridLength, GridUnitType,
                            ...
                            DataTemplate, FrameworkElementFactory)
...
from System.Windows.Media import SolidColorBrush, Colors, BrushConverter
...
# ============================================================================
# VERSION COMPATIBILITY HELPERS  (Revit 2024 / 2025 / 2026 / 2027)
# ============================================================================

def _eid_int(element_id):
    """Return an ElementId's integer value across Revit versions.

    Revit 2024+ deprecated ElementId.IntegerValue (Int32) in favour of
    ElementId.Value (Int64). IntegerValue throws on large 64-bit ids in 2025+.
    Try .Value first, fall back to .IntegerValue for 2023 and earlier.
    """
...
def _brush(hex_color):
    ...
    Color.FromRgb / Color.FromArgb are unreliable in IronPython; BrushConverter
    is the safe path used across the pyDQT suite.
    """
    try:
...
def _brush_from_rgb(r, g, b):
    """Create a SolidColorBrush from RGB ints via a hex string (IronPython safe)."""
    try:
        hex_color = "#{:02X}{:02X}{:02X}".format(int(r), int(g), int(b))
    except:
        return SolidColorBrush(Colors.Gray)
    return _brush(hex_color)


# ============================================================================
# CONFIGURATION  (DQT branding standard)
# ============================================================================

class Config(object):
    """Color scheme and settings"""
    PRIMARY_COLOR = "#F0CC88"      # gold header
    SECONDARY_COLOR = "#E5B85C"
    BACKGROUND_COLOR = "#FFFFFF"   # white main background (synced)
    BORDER_COLOR = "#D4B87A"
    TEXT_DARK = "#5D4E37"
    TEXT_LIGHT = "#888888"
...
# ============================================================================
# MATERIAL ITEM (Data Model)
# ============================================================================

class MaterialItem(INotifyPropertyChanged):
    """Wrapper class for Material with WPF binding support"""
    ...
    def __init__(self, material):
        ...
        self._name = material.Name if material.Name else "Unnamed"
        self._id = _eid_int(material.Id)

        # Get material category
        try:
            self._category = material.MaterialCategory if material.MaterialCategory else "None"
        except:
            self._category = "None"
        ...
        try:
            ...
            self._color_brush = _brush_from_rgb(color.Red, color.Green, color.Blue)
        except:
            self._color = "N/A"
            self._color_brush = SolidColorBrush(Colors.Gray)

        # Get transparency
        try:
            self._transparency = "{}%".format(int(material.Transparency))
        except:
    ...
    def is_selected(self, value):
        if self._is_selected != value:
            ...
            self.OnPropertyChanged("select_mark")

    @property
    def select_mark(self):
        """Text-based checkbox display (read-only DataGrid is IronPython-safe)."""
        return u"\u2611" if self._is_selected else u"\u2610"  # checked / empty box
...
class BatchRenameDialog(Window):
    ...
    def _build_ui(self):
        self.Title = "Batch Rename Materials"
        self.Width = 600
        self.Height = 550
        ...
        info = TextBlock()
        info.Text = "{} material(s) selected for renaming".format(len(self.items))
        info.FontSize = 12
        info.Foreground = _brush(Config.TEXT_LIGHT)
    ...
    def _create_header(self):
        border = Border()
        border.Background = _brush(Config.PRIMARY_COLOR)
        ...
        title.Foreground = _brush(Config.TEXT_DARK)
    ...
    def _create_tabs(self):
        ...
        tab6 = TabItem()
        tab6.Header = "Material Info"
        tab6.Content = self._create_material_info_tab()
    ...
    def _create_material_info_tab(self):
        """Create Material Info tab - add category/color to name"""
        panel = StackPanel()
        panel.Margin = Thickness(10)

        title = TextBlock()
        title.Text = "Add material properties to name:"
        title.FontWeight = FontWeights.SemiBold
        title.Margin = Thickness(0, 0, 0, 10)
        panel.Children.Add(title)

        self.add_category_check = CheckBox()
        self.add_category_check.Content = "Add Category (Concrete, Glass...)"
        ...
        self.add_color_check = CheckBox()
        self.add_color_check.Content = "Add Color (RGB values)"
        self.add_color_check.Margin = Thickness(0, 0, 0, 10)
        self.add_color_check.Checked += lambda s, e: self._update_preview()
        self.add_color_check.Unchecked += lambda s, e: self._update_preview()
        panel.Children.Add(self.add_color_check)

        self.remove_current_name_check = CheckBox()
        self.remove_current_name_check.Content = "Remove current name (use only material info)"
        self.remove_current_name_check.Margin = Thickness(0, 5, 0, 15)
        self.remove_current_name_check.Foreground = _brush(Config.ERROR_COLOR)
    ...
    def _create_preview_section(self):
        ...
        label.Background = _brush(Config.PRIMARY_COLOR)
        label.Foreground = _brush(Config.TEXT_DARK)
        Grid.SetRow(label, 0)
        grid.Children.Add(label)

        scroll = ScrollViewer()
        scroll.VerticalScrollBarVisibility = System.Windows.Controls.ScrollBarVisibility.Auto
        scroll.Background = _brush(Config.WHITE)
    ...
    def _create_buttons(self):
        ...
        apply_btn.Background = _brush(Config.SUCCESS_COLOR)
        apply_btn.Foreground = _brush(Config.WHITE)
    ...
    def _apply_rules_to_name(self, name, index, item=None):
        ...
        # Tab 5: Material Info
            ...
            if item:
                ...
                if self.add_color_check.IsChecked:
                    color_simple = item.color.replace("RGB(", "").replace(")", "").replace(",", "-")
                    info_parts.append(color_simple)
    ...
    def _update_preview(self, sender=None, args=None):
        ...
        if changed_count > 0:
            ...
            self.preview_text.Foreground = _brush(Config.TEXT_BODY)
        else:
            self.preview_text.Text = "No changes will be made with current settings"
            self.preview_text.Foreground = _brush(Config.TEXT_LIGHT)
    ...
    def _on_apply(self, sender, args):
        ...
        t = DB.Transaction(doc, "DQT - Batch Rename Materials")
        t.Start()
        ...
        try:
            ...
            if fail_count:
                msg += "\nFailed (duplicate/invalid name): {}".format(fail_count)
...
class MaterialManagerWindow(Window):
    """Material Manager with Sheet Manager style UI"""
    ...
    def _create_header(self):
        ...
        title = TextBlock()
        title.Text = "Material Manager v1.1"
        title.FontSize = 17
        title.FontWeight = FontWeights.Bold
        title.Foreground = _brush(Config.TEXT_DARK)
        panel.Children.Add(title)
        ...
        subtitle.Foreground = _brush(Config.TEXT_DARK)
    ...
    def _create_card(self, label, value, margin_index, value_color=None):
        ...
        label_text.Foreground = _brush(Config.TEXT_LIGHT)
        ...
        if value_color:
            value_text.Foreground = _brush(value_color)
    ...
    def _create_left_panel(self):
        ...
        for item_text in ["All Materials", "In Use Only", "Unused Only"]:
            item = ComboBoxItem()
        ...
        self.category_combo.Padding = Thickness(6, 4, 6, 4)
        self.category_combo.Margin = Thickness(0, 0, 0, 10)

        item = ComboBoxItem()
        item.Content = "All Categories"
        self.category_combo.Items.Add(item)
        self.category_combo.SelectedIndex = 0
        self.category_combo.SelectionChanged += self._on_filter_changed
        ...
        btn_all.Background = _brush(Config.WHITE)
        ...
        btn_clear.Background = _brush(Config.WHITE)
        ...
        btn_unused.Background = _brush(Config.WHITE)
    ...
    def _create_datagrid(self):
        ...
        # Read-only DataGrid is IronPython-safe; DataGridCheckBoxColumn in edit
        # mode causes fatal crashes, so selection is via a text mark column that
        # is toggled with MouseDoubleClick.
        ...
        grid.Background = _brush(Config.WHITE)
        grid.BorderBrush = _brush(Config.BORDER_COLOR)
        grid.GridLinesVisibility = System.Windows.Controls.DataGridGridLinesVisibility.Horizontal
        grid.HorizontalGridLinesBrush = _brush(Config.GRID_LINE)
        grid.RowBackground = _brush(Config.WHITE)
        grid.AlternatingRowBackground = _brush(Config.ROW_ALT_COLOR)

        # Selection mark column (text-based checkbox, read-only, double-click toggles)
        col_check = DataGridTextColumn()
        col_check.Header = u"\u2713"
        col_check.Binding = Binding("select_mark")
        col_check.Width = DataGridLength(40)
        grid.Columns.Add(col_check)

        col_name = DataGridTextColumn()
        col_name.Header = "Material Name"
        col_name.Binding = Binding("name")
        col_name.Width = DataGridLength(250)
        grid.Columns.Add(col_name)

        col_cat = DataGridTextColumn()
        col_cat.Header = "Category"
        col_cat.Binding = Binding("category")
        col_cat.Width = DataGridLength(120)
        grid.Columns.Add(col_cat)

        # Color column with preview
        col_color = DataGridTemplateColumn()
        col_color.Header = "Color"
        col_color.Width = DataGridLength(150)

        # Create template programmatically
        factory = FrameworkElementFactory(StackPanel)
        factory.SetValue(StackPanel.OrientationProperty, Orientation.Horizontal)

        border_factory = FrameworkElementFactory(Border)
        border_factory.SetValue(Border.WidthProperty, 16.0)
        border_factory.SetValue(Border.HeightProperty, 16.0)
        border_factory.SetValue(Border.MarginProperty, Thickness(2))
        border_factory.SetValue(Border.BorderThicknessProperty, Thickness(1))
        border_factory.SetValue(Border.BorderBrushProperty, SolidColorBrush(Colors.Gray))
        border_factory.SetBinding(Border.BackgroundProperty, Binding("color_brush"))
        factory.AppendChild(border_factory)

        text_factory = FrameworkElementFactory(TextBlock)
        text_factory.SetValue(TextBlock.MarginProperty, Thickness(5, 0, 0, 0))
        text_factory.SetValue(TextBlock.VerticalAlignmentProperty, VerticalAlignment.Center)
        text_factory.SetBinding(TextBlock.TextProperty, Binding("color"))
        ...
        col_trans = DataGridTextColumn()
        col_trans.Header = "Transparency"
        col_trans.Binding = Binding("transparency")
        ...
        grid.MouseDoubleClick += self._on_grid_double_click
    ...
    def _create_action_buttons(self):
        ...
        primary = _brush(Config.PRIMARY_COLOR)
        orange = _brush(Config.WARNING_COLOR)
        red = _brush(Config.ERROR_COLOR)
        white = _brush(Config.WHITE)
    ...
    def _create_button(self, text, bg_brush, white_fg=False):
        ...
        if white_fg:
            btn.Foreground = _brush(Config.WHITE)
    ...
    def _create_tips(self):
        ...
        tips = TextBlock()
        tips.Text = ("Double-click a row to toggle its checkbox  |  "
                     "Deleting materials in use may affect existing elements  |  "
                     "Use Batch Rename for multiple materials")
        tips.FontSize = 10
        tips.Foreground = _brush(Config.TEXT_LIGHT)
        grid.Children.Add(tips)

        btn_close = Button()
        btn_close.Content = "Close"
        btn_close.Padding = Thickness(15, 5, 15, 5)
        btn_close.Background = _brush(Config.WHITE)
    ...
    def _create_footer(self):
        ...
        text = TextBlock()
        text.Text = u"Dang Quoc Truong - DQT (c) 2026"
        text.FontSize = 10
        text.FontWeight = FontWeights.SemiBold
        text.HorizontalAlignment = HorizontalAlignment.Center
        text.Foreground = _brush(Config.TEXT_DARK)
    ...
    def _load_data(self):
        ...
        try:
            ...
            # Calculate usage
            calculate_material_usage(doc, self.all_items)
            ...
            print("Error loading materials: {}".format(str(ex)))
    ...
    def _apply_filters(self):
        ...
        category_filter = ""
        if self.category_combo and self.category_combo.SelectedIndex > 0:
            selected = self.category_combo.SelectedItem
            if selected:
                category_filter = selected.Content
    ...
    def _on_selection_changed(self, sender, args):
        # Mirror DataGrid row selection into the is_selected flag so both the
        # standard highlight and the text-mark column stay in sync.
        try:
    ...
    def _on_grid_double_click(self, sender, args):
        # Double-click toggles the checkbox mark on the currently focused item.
        try:
            item = self.data_grid.SelectedItem
            if item is not None and item in self.filtered_items:
    ...
    def _on_rename(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select one material to rename!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return

        if len(selected) > 1:
            MessageBox.Show("Please select only ONE material to rename!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        ...
        t = DB.Transaction(doc, "DQT - Rename Material")
        t.Start()
    ...
    def _on_batch_rename(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select at least one material to batch rename!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
    ...
    def _on_duplicate(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select at least one material to duplicate!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        ...
        for item in selected:
            new_name = forms.ask_for_string(
                prompt="Enter name for duplicate of '{}':".format(item.name),
                default=item.name + " Copy",
            ...
            t = DB.Transaction(doc, "DQT - Duplicate Material")
            t.Start()

            try:
                item.material.Duplicate(new_name)
                t.Commit()
                success_count += 1
            except Exception as ex:
                if t.HasStarted() and not t.HasEnded():
                    t.RollBack()
                MessageBox.Show("Failed to duplicate '{}':\n{}".format(item.name, str(ex)),
                              "Error", MessageBoxButton.OK, MessageBoxImage.Error)
    ...
    def _on_delete(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select at least one material to delete!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        ...
        if in_use:
            ...
            result = MessageBox.Show(msg, "Materials In Use",
        ...
        result = MessageBox.Show(
            "Delete {} material(s)?".format(len(can_delete)),
            "Confirm Delete",
        ...
        t = DB.Transaction(doc, "DQT - Delete Materials")
        t.Start()
...
if __name__ == '__main__':
    try:
        ...
        MessageBox.Show(
            "Error starting Material Manager:\n\n{}".format(str(e)),
            "Error",
            MessageBoxButton.OK,
