# -*- coding: utf-8 -*-
"""
Text Note Type Manager (Standalone)
Manage Text Note Types with beautiful UI - Sheet Manager Style

Copyright (c) 2026 Dang Quoc Truong (DQT)
All rights reserved.
"""
__title__ = "Text Note Type\nManage"
__author__ = "Dang Quoc Truong (DQT)"
__doc__ = "Manage and rename Text Note Types - By DQT"
...
def _eid_int(eid):
    """Get integer value from ElementId - compatible Revit 2024-2027.
    Revit 2025+ removed IntegerValue, uses Value (int64) instead."""
    try:
        return eid.Value          # Revit 2025/2026/2027
...
class Config(object):
    ...
    def hex_to_brush(hex_color):
        """Convert hex to SolidColorBrush - reliable in IronPython"""
        return System.Windows.Media.BrushConverter().ConvertFromString(hex_color)
...
class TextNoteTypeItem(object):
    ...
    def __init__(self, text_type):
        ...
        # Get name
        name_param = text_type.get_Parameter(DB.BuiltInParameter.SYMBOL_NAME_PARAM)
        self._name = name_param.AsString() if name_param else "Unnamed"
        
        self._id = _eid_int(text_type.Id)
    ...
    def _get_font_name(self):
        """Get font name"""
        try:
            font_param = self._text_type.get_Parameter(DB.BuiltInParameter.TEXT_FONT)
            if font_param and font_param.HasValue:
                return font_param.AsString()
        except Exception:
            pass
        return "Unknown"
...
class BatchRenameDialog(Window):
    ...
    def __init__(self, items, parent_window):
        ...
        # Type Info tab
        self.add_size_check = None
        self.add_font_check = None
    ...
    def _build_ui(self):
        self.Title = "Batch Rename Text Note Types"
        self.Width = 600
        self.Height = 550
        ...
        info = TextBlock()
        info.Text = "{} type(s) selected for renaming".format(len(self.items))
        info.FontSize = 12
    ...
    def _create_tabs(self):
        ...
        tab6 = TabItem()
        tab6.Header = "Type Info"
        tab6.Content = self._create_type_info_tab()
    ...
    def _create_type_info_tab(self):
        """Create Type Info tab - add text size/font to name"""
        panel = StackPanel()
        panel.Margin = Thickness(10)
        
        title = TextBlock()
        title.Text = "Add type properties to name:"
        title.FontWeight = FontWeights.SemiBold
        title.Margin = Thickness(0, 0, 0, 10)
        panel.Children.Add(title)
        
        self.add_size_check = CheckBox()
        self.add_size_check.Content = "Add Text Size (2.5mm, 3mm...)"
        self.add_size_check.Margin = Thickness(0, 5, 0, 5)
        self.add_size_check.Checked += lambda s, e: self._update_preview()
        self.add_size_check.Unchecked += lambda s, e: self._update_preview()
        panel.Children.Add(self.add_size_check)
        
        self.add_font_check = CheckBox()
        self.add_font_check.Content = "Add Font Name (Arial, Times...)"
        self.add_font_check.Margin = Thickness(0, 0, 0, 10)
        self.add_font_check.Checked += lambda s, e: self._update_preview()
        self.add_font_check.Unchecked += lambda s, e: self._update_preview()
        panel.Children.Add(self.add_font_check)
        
        self.remove_current_name_check = CheckBox()
        self.remove_current_name_check.Content = "Remove current name (use only type info)"
        self.remove_current_name_check.Margin = Thickness(0, 5, 0, 15)
        self.remove_current_name_check.Foreground = Config.hex_to_brush(Config.ERROR_COLOR)
    ...
    def _create_preview_section(self):
        ...
        label.Background = Config.hex_to_brush(Config.PRIMARY_COLOR)
        Grid.SetRow(label, 0)
        grid.Children.Add(label)
        
        scroll = ScrollViewer()
        scroll.VerticalScrollBarVisibility = System.Windows.Controls.ScrollBarVisibility.Auto
        scroll.Background = Config.hex_to_brush(Config.WHITE)
    ...
    def _apply_rules_to_name(self, name, index, item=None):
        ...
        # Tab 5: Type Info
            ...
            if item:
                ...
                if self.add_size_check.IsChecked:
                    info_parts.append(item.text_size)
                
                if self.add_font_check.IsChecked:
                    info_parts.append(item.font)
    ...
    def _update_preview(self, sender=None, args=None):
        ...
        if changed_count > 0:
            ...
            self.preview_text.Foreground = Config.hex_to_brush("#333333")
        else:
            self.preview_text.Text = "No changes will be made with current settings"
            self.preview_text.Foreground = Config.hex_to_brush(Config.TEXT_LIGHT)
    ...
    def _on_apply(self, sender, args):
        ...
        t = DB.Transaction(doc, "DQT - Batch Rename Text Note Types")
        t.Start()
...
class TextNoteTypeManagerWindow(Window):
    ...
    def _build_ui(self):
        self.Title = "Text Note Type Manager"
        self.Width = 1100
        self.Height = 700
        self.WindowStartupLocation = System.Windows.WindowStartupLocation.CenterScreen
        self.Background = Config.hex_to_brush(Config.BACKGROUND_COLOR)
    ...
    def _create_left_panel(self):
        ...
        for item_text in ["All Types", "In Use Only", "Unused Only"]:
            item = ComboBoxItem()
        ...
        btn_unused.Background = Config.hex_to_brush(Config.WHITE)
    ...
    def _load_data(self):
        ...
        try:
            ...
            for text_type in collector:
                try:
                    item = TextNoteTypeItem(text_type)
            ...
            # Calculate usage
            calculate_textnote_usage(doc, self.all_items)
            
            self._apply_filters()
            self._update_stats()
            
        except Exception as ex:
            print("Error loading text note types: {}".format(str(ex)))
            MessageBox.Show("Failed to load text note types:\n\n{}".format(str(ex)),
    ...
    def _apply_filters(self):
        ...
        # Use DataTable for reliable WPF DataGrid binding
        dt = self._build_datatable(self.filtered_items)
        self.data_grid.ItemsSource = dt.DefaultView
    ...
    def _on_rename(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select one type to rename!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        
        if len(selected) > 1:
            MessageBox.Show("Please select only ONE type to rename!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        ...
        t = DB.Transaction(doc, "DQT - Rename Text Note Type")
        t.Start()
    ...
    def _on_batch_rename(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select at least one type to batch rename!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
    ...
    def _on_duplicate(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select at least one type to duplicate!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        
        t = DB.Transaction(doc, "DQT - Duplicate Text Note Types")
        t.Start()
        ...
        try:
            ...
            for item in selected:
                try:
                    new_type = item.text_type.Duplicate("Copy of " + item.name)
    ...
    def _on_delete(self, sender, args):
        ...
        if not selected:
            MessageBox.Show("Please select at least one type to delete!",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        
        # Prevent deleting all types - Revit requires at least 1
        if len(selected) >= len(self.all_items):
            MessageBox.Show("Cannot delete ALL Text Note Types.\nRevit requires at least one type to remain.",
                          "Warning", MessageBoxButton.OK, MessageBoxImage.Warning)
            return
        ...
        if in_use:
            ...
            result = MessageBox.Show(
                "Delete {} type(s)?".format(len(selected)),
                "Confirm Delete",
        ...
        t = DB.Transaction(doc, "DQT - Delete Text Note Types")
        t.Start()
...
if __name__ == '__main__':
    try:
        window = TextNoteTypeManagerWindow()
        window.ShowDialog()
        ...
        MessageBox.Show(
            "Error starting Text Note Type Manager:\n\n{}".format(str(e)),
            "Error",
            MessageBoxButton.OK,
